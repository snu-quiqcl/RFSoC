...
def generate_xilinx_dds_ip(folder_directory, total_dds_num):
    tcl_code = ''
...
def generate_xilinx_block_design(folder_directory):
    tcl_code = ''
    return tcl_code

def generate_customized_ip(folder_directory):
    tcl_code = ''
...
def generate_xilinx_fifo_generator(folder_directory, total_fifo_num):
    tcl_code = ''
...
def make_tcl(folder_directory,prj_name,part_name,board_path,board_name,file_type):
    ...
    tcl_code += '\n'
    # tcl_code += generate_xlilinx_Zynq_MPSoC(folder_directory)
    # tcl_code += '\n'
    for i in range(16):
        tcl_code += generate_xilinx_dds_ip(folder_directory,i)
        tcl_code += '\n'
        
    for i in range(1):
        tcl_code += generate_xilinx_fifo_generator(folder_directory,i)
        tcl_code += '\n'
    # tcl_code += open_vivado(folder_directory, prj_name)
    # tcl_code += '\n'
    
    tcl_code += generate_customized_ip(folder_directory)
    tcl_code += '\n'
...
def open_vivado(folder_directory,prj_name):
    tcl_code = ''
...
if __name__ == "__main__":
    ...
    prj_name = "DAC_Controller_ip_test"
